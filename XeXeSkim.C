...
*/
void XeXeSkim(TString input_file, TString ouputfile, int isMC){
	...
	const Int_t nMaxTrack = 50000;		// Maximum number of tracks in an event
	
	// Define trees to be read from the files
	const int nJetTrees = 3;
	TChain *heavyIonTree = new TChain("hiEvtAnalyzer/HiTree");
	TChain *hltTree = new TChain("hltanalysisReco/HltTree");
	TChain *skimTree = new TChain("skimanalysis/HltTree");
	TChain *jetTree[nJetTrees];
	jetTree[0] = new TChain("ak4PFJetAnalyzer/t");
	jetTree[1] = new TChain("akCs4PFJetAnalyzer/t");
	jetTree[2] = new TChain("akPu4PFJetAnalyzer/t");
	...
	// HLT
	TBranch *HLT_HIL1SingleJet120_part1;		// Branch for L1 jet 120 filter bit part1
	TBranch *HLT_HIL1SingleJet120_part2;		// Branch for L1 jet 120 filter bit part2
	
	// Leaves for the HLT tree
	Int_t HIL1SingleJet120_part1;				// Filter bit for L1 jet 120 filter bit part1
	Int_t HIL1SingleJet120_part2;				// Filter bit for L1 jet 120 filter bit part2
	...
	TBranch *hfCoincidenceBranch;				// Branch for energy recorded one HF tower above threshold of 3 GeV on each side
	...
	hltTree->SetBranchStatus("HLT_HIL1SingleJet120_part1_v1",1);
	hltTree->SetBranchAddress("HLT_HIL1SingleJet120_part1_v1",&HIL1SingleJet120_part1,&HLT_HIL1SingleJet120_part1);
	hltTree->SetBranchStatus("HLT_HIL1SingleJet120_part2_v1",1);
	hltTree->SetBranchAddress("HLT_HIL1SingleJet120_part2_v1",&HIL1SingleJet120_part2,&HLT_HIL1SingleJet120_part2);
	...
	skimTree->SetBranchStatus("phfCoincFilter3",1);
	skimTree->SetBranchAddress("phfCoincFilter3", &hfCoincidenceFilterBit, &hfCoincidenceBranch);
	...
	hltTreeOutput->Branch("HLT_HIL1SingleJet120_part1_v1",&HIL1SingleJet120_part1,"HLT_HIL1SingleJet120_part1_v1/I");
	hltTreeOutput->Branch("HLT_HIL1SingleJet120_part2_v1",&HIL1SingleJet120_part2,"HLT_HIL1SingleJet120_part2_v1/I");
	...
	skimTreeOutput->Branch("phfCoincFilter3", &hfCoincidenceFilterBit, "phfCoincFilter3/I");
	...
	for(int iEvent = 0; iEvent < nEvents; iEvent++) {
		...
		// if( iEvent == 200 ) break; //used for tests
		// ========================================== //
		...
		for(int iJetType = 0; iJetType < nJetTrees; iJetType++){jetTree[iJetType]->GetEntry(iEvent);}
    	...
    	for(int iTrack = 0; iTrack < nTracks; iTrack++){
    		...
    		if(fabs(trackPtErrorArray[iTrack]/trackPtArray[iTrack]) > 0.2) passTrackCuts = false;
	...
	gDirectory->cd("../");
	const char *jetDirectories[] = {"ak4PFJetAnalyzer","akCs4PFJetAnalyzer","akPu4PFJetAnalyzer"};
	for(int iJetType = 0; iJetType < nJetTrees; iJetType++){
...
int main(int argc, char** argv){
				...
				int mc = atoi(argv[3]);
				XeXeSkim(firstArgument,outfile,mc);
}
